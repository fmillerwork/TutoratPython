...
del liste[-1] #supprime le dernier élément de la liste (car indice -1).
print(liste)
...
#ATTENTION : les méthodes appliquées aux listes modifie la liste courante, contrairement aux méthodes appliquées aux string (elles créent un nouveau string)
s = "Xyz"

s = s.replace('X','x')
s.replace('X','x') #FAUX (ne fait sert)
L = liste.sort()  #trie la liste Liste et la copie dans une autre liste L
liste.sort() #trie la liste liste
